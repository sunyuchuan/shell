        ...
        if [ -d "$1/$file" ]; then
            ...
            if [ "${file##*.}"x = "mp3"x ]||[ "${file##*.}"x = "MP3"x ] ||
               ...
               [ "${file##*.}"x = "m4a"x ]||[ "${file##*.}"x = "M4A"x ] ;then
                ...
                ffmpeg -i $1/$file -filter_complex "[0:a]alimiter=level_in=$gain:level_out=1:limit=-1dB:attack=5:release=200[out]" -map "[out]" -y $2/$file.wav 1>/dev/null 2>/dev/null

                end=`date +%s`
                dif=$[ end - start ]
                echo "file name ---> $1/$file gain: $gain cost time $dif sec"
            fi
        fi
    done
}
